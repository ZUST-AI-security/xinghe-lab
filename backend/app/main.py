...
@app.middleware("http")
async def log_422_requests(request: Request, call_next):
    """临时调试：记录 422 请求的详细信息"""
    ...
    if response.status_code == 422:
        ...
        logger.error(f"422 DEBUG - Path: {request.url.path} - Response: {body_str}")
        # 重新构建响应
        from fastapi.responses import Response
        return Response(content=body_bytes, status_code=422, 
