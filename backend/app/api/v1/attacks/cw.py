"""
C&W (Carlini & Wagner L2) attack API routes.

Provides two execution modes:
  POST /cw/run     — synchronous (quick demo mode with capped iterations)
  POST /cw/submit  — asynchronous via Celery (returns task_id for polling)
...
from app.schemas.attacks.cw import (
    CWAttackRequest,
    CWAttackResponse,
    CWAsyncTaskResponse,
...
# In-process model cache for sync endpoint
...
async def run_cw_sync(
    ...
    """
    Run C&W attack synchronously in fast-demo mode (capped iterations).
    Use /submit for full-quality async execution.
    """
...
async def get_cw_params_schema():
    """Return the C&W parameter schema for frontend rendering."""
    try:
        from app.algorithms.registry import get as get_algorithm
        algo_cls = get_algorithm("cw")
        if algo_cls is None:
            raise HTTPException(status_code=404, detail="C&W algorithm not registered")
...
async def get_cw_history(
    page: int = Query(1, ge=1),
    size: int = Query(10, ge=1, le=100),
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db),
):
    """Get C&W attack history for the current user."""
    # TODO: implement DB history storage
    return CWHistoryListResponse(histories=[], total=0, page=page, size=size, pages=0)
...
@router.get("/stats")
async def get_cw_stats(
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db),
):
    """Get C&W attack statistics for the current user."""
