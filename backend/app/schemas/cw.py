...
class CWAttackResponse(BaseModel):
    ...
    original_prediction: Optional[Dict[str, Any]] = Field(None, description="原图分类结果：包含类别ID、类别名称、置信度")
    adversarial_prediction: Optional[Dict[str, Any]] = Field(None, description="对抗样本分类结果：包含类别ID、类别名称、置信度")
    success: bool = Field(..., description="攻击是否成功")
    time_elapsed: float = Field(..., description="攻击执行耗时（秒）")
    metadata: Dict[str, Any] = Field(..., description="附加元数据：包含L2范数、迭代次数、噪声幅度等")
