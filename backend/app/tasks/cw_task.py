...
@celery_app.task(bind=True, name="app.tasks.cw_task.run_cw_attack", soft_time_limit=300, time_limit=400)
def run_cw_attack(self, image: str, params: Dict[str, Any], model_name: str, user_id: int):
    ...
    adv_np = None
    heatmap_np = None
    noise_visual = None  # 初始化 noise_visual
    noise_np = None      # 初始化 noise_np
    ...
    try:
        ...
        # 6. 运行攻击（使用快速参数避免超时）
        # 强制使用快速模式参数，覆盖用户设置以确保在时限内完成
        fast_params = {
            'c': max(params.get('c', 1.0), 5.0),      # 至少5.0，提高攻击成功率
            'kappa': params.get('kappa', 0.0),           # 保持用户设置
            'lr': max(params.get('lr', 0.01), 0.05),     # 至少0.05，加快收敛
            'max_iter': 300,                              # 提升至300，确保攻击成功率
            'binary_search_steps': 3,                     # 提升至3，确保能找到最优c值
            'targeted': params.get('targeted', False),      # 保持用户设置
            'abort_early': True,                           # 启用提前终止
            'early_stop_iters': 5                           # 强制最小值
        }
        
        logger.info(f"使用快速攻击参数: {fast_params}")
        
        # 运行攻击 - 接收3个值：原始图像、对抗样本、元数据
        ...
        original_np = original_np.transpose(1, 2, 0)  # CHW -> HWC
        ...
        result = {
            ...
            "per_sample_results": metadata.get('per_sample_results', []),  # 添加每样本详细结果
            "metadata": {
                'l2_norm': metadata.get('avg_l2_norm', metadata.get('perturbation_norm', 0)),
        ...
        # 更新任务状态为失败 - 核心修复：确保符合Celery标准格式
        self.update_state(
            state="FAILURE",
            meta={
                "exc_type": type(e).__name__,           # 异常类型名（必需）
                "exc_message": [str(e)],               # 异常消息列表（标准格式）
                "error": str(e),                       # 向后兼容
                "progress": 0,
                "custom_error": "C&W攻击执行期间发生崩溃",
                "traceback": None  # 避免traceback序列化问题
            }
        )
        
        # 抛出标准异常，不要抛出复杂的自定义异常
        raise RuntimeError(f"C&W Task Failed: {str(e)}")
    finally:
        ...
        del adv_np, heatmap_np, noise_visual, noise_np  # 添加噪声图变量
        _release_torch_memory()

@celery_app.task(name="app.tasks.cw_task.cleanup_cw_results")
...
@celery_app.task(name="app.tasks.cw_task.get_task_statistics")
